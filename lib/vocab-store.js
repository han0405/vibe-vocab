...
function readRate(cwd) {
  ...
  for (const f of [path.join(dir, RATE_FILE), path.join(claudeDir, RATE_FILE)]) {
    try {
...
function readLevel(cwd) {
  ...
  const claudeDir =
    process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude');
  for (const f of [path.join(dir, LEVEL_FILE), path.join(claudeDir, LEVEL_FILE)]) {
    try {
      const raw = fs.readFileSync(f, 'utf8').trim();
...
function readKnownTerms(cwd) {
  ...
  const seen = new Set();
  const out = [];
  for (const f of [path.join(dir, KNOWN_FILE), path.join(claudeDir, KNOWN_FILE)]) {
    let raw = '';
    try {
...
// whole row (and its escaped `\|`s) into the log. `।` / `۔` (Hindi/Urdu danda)
// and `؟` (Arabic question mark) keep non-Latin scripts from over-running;
// mid-clause commas (`，` `،`) are deliberately left out.
const CTX_MARKS = ['\n', '。', '！', '？', '；', '।', '۔', '؟', '. ', '! ', '? ', '|', '：', ':'];
